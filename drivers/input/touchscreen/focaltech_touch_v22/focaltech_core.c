...
#endif
//Fred add for MBA
unsigned int FTS_CHIP_TYPE = 0;
unsigned int FTS_CHIP_IDC = 0;
unsigned int FTS_HID_SUPPORTTED = 0;
...
#if FTS_GESTURE_EN
static bool support_tgesture = false;
#endif
...
int fts_reset_proc(int hdelayms)
{
    ...
    if (!gpio_is_valid(fts_data->pdata->reset_gpio)) {
        FTS_ERROR("reset_gpio is not valid");
        return -1;
    }
...
#if FTS_ESDCHECK_EN
if (fts_data->pdata->esd_check_enable)
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    ret = of_property_read_u32(np, "focaltech,chiptype", &temp_val);
    if (0 == ret) {
            pdata->chip_type = temp_val;
    } else {
        FTS_ERROR("Unable to get chip_type");
        pdata->chip_type = _FT8716;
    }
    FTS_CHIP_TYPE = pdata->chip_type;
    FTS_CHIP_IDC = IC_TO_FTS_CHIP_IDC(pdata->chip_type);
    FTS_HID_SUPPORTTED = IC_TO_FTS_HID_SUPPORTTED(pdata->chip_type);

    pdata->multiple_ic = of_property_read_bool(np, "focaltech,multiple-ic");
    if (pdata->multiple_ic)
	FTS_INFO("multiple ic supported!");

    pdata->fw_auto_update = of_property_read_bool(np, "focaltech,fw-auto-update");
    if (pdata->fw_auto_update)
	FTS_INFO("TP FW auto upgrade enabled!");
    else
	FTS_INFO("TP FW auto upgrade disabled!");

    pdata->display_cfg_auto_update = of_property_read_bool(np, "focaltech,display-cfg-auto-update");
    if (pdata->display_cfg_auto_update)
	FTS_INFO("display config auto upgrade enabled!");
    else
	FTS_INFO("display config auto upgrade disabled!");

#if FTS_GESTURE_EN
    support_tgesture = of_property_read_bool(np, "focaltech,support-tp-gesture");
    if (support_tgesture)
	FTS_INFO("TP support tgesture!");
    else
	FTS_INFO("TP unsupport tgesture!");
#endif

    pdata->esd_check_enable = of_property_read_bool(np, "focaltech,esdcheck-enable");
    if (pdata->esd_check_enable)
	FTS_INFO("ESD check enabled!");
    else
	FTS_INFO("ESD check disabled!");

    global_debug_flag = of_property_read_bool(np, "focaltech,enable-debug-log");
    if (global_debug_flag)
	FTS_INFO("enable debug log output!");

    FTS_INFO("max touch number:%d, irq gpio:%d, reset gpio:%d, chip_type:0x%x, FTS_CHIP_IDC:%d, FTS_HID_SUPPORTTED:%d",
             pdata->max_touch_number, pdata->irq_gpio, pdata->reset_gpio, pdata->chip_type, FTS_CHIP_IDC, FTS_HID_SUPPORTTED);
...
extern int sensitive_notifier_register(struct notifier_block *nb);
extern struct fts_mode_flag g_fts_mode_flag;
extern bool high_sens_mode;
static int focal_high_sensitive_setting (struct notifier_block *nb, unsigned long enable, void *arg)
{
	int ret = 0;
	FTS_FUNC_ENTER();
	...
	if (enable) {
		if (!g_fts_mode_flag.fts_cover_mode_flag) {
			...
			ret = fts_enter_cover_mode(fts_data->client, true);
			if (ret >= 0) {
		...
		if (g_fts_mode_flag.fts_cover_mode_flag) {
			...
			ret = fts_enter_cover_mode(fts_data->client, false);
			if (ret >= 0) {
				g_fts_mode_flag.fts_cover_mode_flag = false;
				high_sens_mode = false;
			}
		}
	}
...
static struct notifier_block high_sensitive_nb = {
	.notifier_call = focal_high_sensitive_setting,
};
#endif
#if defined(CONFIG_FB)
...
int fts_set_tp_sleep_mode(void)
{
    int ret = 0;

    FTS_FUNC_ENTER();
    /* TP enter sleep mode */
    ret = fts_i2c_write_reg(fts_data->client, FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP_VALUE);
    if (ret < 0)
        FTS_ERROR("set TP to sleep mode fail, ret=%d", ret);
    else
        FTS_ERROR("set TP to sleep mode done");
    FTS_FUNC_EXIT();
...
#if FTS_GESTURE_EN
if (support_tgesture)
    get_and_init_TGesture_dev();
#else
//add to avoid compiling error
if (0) {
    get_and_init_TGesture_dev();
    TGesture_dev_report(0);
}
#endif
#endif
...
//    sensitive_notifier_register(&high_sensitive_nb);

#if defined(CONFIG_FB)
#ifdef CONFIG_TINNO_DRV_OPTIMIZE
    INIT_WORK(&ts_data->fb_notify_work, fb_notify_resume_work);
#endif
